...
4. After these commands the virtual interface is up and running, now we need to edit the files to make the toolchain use this interface for its data
  i.	Update toolchain/simulationConfiguration.xml to have only the IEDs that we want the tool to use
  ii.	Update toolchain/IED1/AttackScenarioConfiguration.xml to reflect the changed interface names from the original (veth1 instead of ens33)
