...
inject_overcurrent_on_line_to_bus(sim, t0=6, duration=3, line_name="L4", factor=2)
inject_overcurrent_on_tx_side(sim, t0=6, duration=3, tx_name="T1", side="hv", factor = 10.0)
sim.add_sampler(sample_ieds())
sim_data = sim.run(s)

# Export CSVs (for ied comm)
ieds = [ied1, ied2, ied100] 
