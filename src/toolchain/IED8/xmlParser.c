...
struct DosAttack* parserDosAttackXML(xmlNode *attackNode) {
					...
					/* Begin to Parse payload Xml node*/
								...
								if (!strcmp(payloadChild->name, "values")) {
									...
									if (strcmp(value, "non-value")) {
										if (!strcmp(payloadChild->name,"stNum")) {
											...
											snprintf(dosAtta->dstAddress, sizeof(dosAtta->dstAddress), "%s", value);
										...
										} else if(!strcmp(payloadChild->name,"confRev")) {
											dosAtta->confRev=atoi(value);
										} else if (!strcmp(payloadChild->name,"dataSet")) {
						...
						while (stopChild) {
							if (stopChild->type == XML_ELEMENT_NODE) {
								...
								else if (!strcmp(stopChild->name, "pps")) {
									char *value = getAttributeValueByName("value", stopChild);
									if (strcmp(value, "non-value")) {
										dosAtta->pps = atoi(value);
									}
								}
								else if (!strcmp(stopChild->name, "interPacketUs")) {
									char *value = getAttributeValueByName("value", stopChild);
									if (strcmp(value, "non-value")) {
										dosAtta->interPacketUs = atoi(value);
									}
								}
	...
	printf("[XML] DoS stop_condition: packetsNum=%d interPacketUs=%d\n",
	dosAtta->stopCondition_packetNum, dosAtta->interPacketUs);
	return dosAtta;
}
