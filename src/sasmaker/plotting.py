...
def plot_one_line(substation, *,
            ...
            if bb._draw_label:
                if slots==1:
                    ax.text(x, y + T*1.2, f"{bb.name}",
                        ha="center", va="bottom", fontsize=fontsize, zorder=100)    
                else:
                    ax.text(x-(slots*busbar_length*0.5), y + T*1.2, f"{bb.name}",
                            ha="left", va="bottom", fontsize=fontsize, zorder=100)
    ...
    for name, ln in substation.lines.items():
        ...
        if label_lines:
            ...
            ax.text(xm, ym, name, fontsize=fontsize, ha="center", va="center", zorder=4)
    ...
    for name, tx in getattr(substation, "txs", {}).items():
        ...
        radius = size*0.45
        ax.text(mx+(2*radius), my-0.02, name, fontsize=fontsize, ha="center", va="bottom", zorder=130)
    ...
    for name, ld in substation.loads.items():
        ...
        if ld._draw_label:
            ax.text(ex, ey - 0.05, name, ha="center", va="top", fontsize=fontsize, zorder=4)
    ...
    for name, cb in substation.cbs.items():
        ...
        if target_kind == "line":
            ...
            ax.text(px+0.05, py-sign*0.02, name, fontsize=fontsize,
            ...
            nx, ny = -uy, ux
            if ny < 0: nx, ny = -nx, -ny
            ax.text(px + nx * 0.03, py - ny * 0.08, name, fontsize=fontsize,
    ...
    for ied_name, ied in substation.ieds.items():
        ...
        if cb_src is not None and getattr(cb_src, "target_kind", "line") == "buslink":
            ...
            ax.text(box_x + box_w/2, box_y + box_h/2, ied_name,
                    ha="center", va="center", fontsize=fontsize, zorder=111)
    ...
    for vt_name, vt in getattr(substation, "vts", {}).items():
        ...
        ax.text(vx + 0.03, vy, vt_name, fontsize=fontsize, va="center", ha="left", zorder=161)
