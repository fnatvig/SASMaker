...
class Load:
    ...
    def set_power(self, p_mw: float, q_mvar: float):
        ...
        pa = self.fa * p_mw; pb = self.fb * p_mw; pc = self.fc * p_mw
        qa = self.fa * q_mvar; qb = self.fb * q_mvar; qc = self.fc * q_mvar
        # temp = random.random()
    ...
    def profile(self, base_p: float, base_q: float, t: float, period: float = 900.0):
        """
