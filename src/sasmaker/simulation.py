...
class Simulation:
    ...
    vary_loads_every: int = 1
    vary_loads_min_gap: int = 5    # minimum seconds between changes
    seed: int = 1
    _next_load_change_t: float = field(default=0.0, init=False)
    ...
    def run(self, s: "Substation") -> pd.DataFrame:
        ...
        random.seed(self.seed)
            ...
            # --- initialize load variation schedule ---
        if self.vary_loads:
            mean = max(1, int(self.vary_loads_every))
            min_gap = max(0, int(self.vary_loads_min_gap))
            # first change happens after a random delay
            self._next_load_change_t = float(
                min_gap + random.expovariate(1.0 / max(mean - min_gap, 1e-9))
            )
        t = 0.0
        while t <= self.t_end + 1e-12:
            ...
            # 1) apply load variation if enabled (optionally only every N steps)
            if self.vary_loads and t >= self._next_load_change_t - 1e-12:
                loads = list(s.loads.values())
                if loads:  # safety
                    ld = random.choice(loads)
                    ld.profile(ld.base_p, ld.base_q, t)
                mean = max(1, int(self.vary_loads_every))
                min_gap = max(0, int(self.vary_loads_min_gap))

                # schedule next change
                wait = min_gap + random.expovariate(1.0 / max(mean - min_gap, 1e-9))
                self._next_load_change_t = float(t + wait)
...
def trigger_ied_cb_trip(sim: "Simulation",
                        ied: Union["IED", str],
                        t0: float,
                        *,
                        label: str | None = None) -> None:
    """
    Assert PTRC.trip=True for a single IED at (snapped) time t0.

    IMPORTANT:
      - This does NOT open the breaker directly.
      - Breaker opening is handled by IED.tick() (one-step delayed open via _prev_trip).
    """
    step0 = round(t0 / sim.dt)
    t_trip = round(step0 * sim.dt, 9)

    def _resolve_ied(s: "Substation"):
        if isinstance(ied, str):
            if ied not in s.ieds:
                raise KeyError(f"IED {ied!r} not found in substation.")
            return s.ieds[ied]
        nm = getattr(ied, "name", None)
        if not nm:
            raise TypeError("ied must be an IED or an IED name (str).")
        if nm not in s.ieds:
            raise KeyError(f"IED {nm!r} not found in substation.")
        return s.ieds[nm]

    def _trip(s: "Substation"):
        the_ied = _resolve_ied(s)
        if getattr(the_ied, "ptrc", None) is None:
            raise ValueError(f"IED {the_ied.name!r} has no PTRC; cannot trip.")
        the_ied.ptrc.set_trip(True)

    ied_label = ied if isinstance(ied, str) else getattr(ied, "name", "IED?")
    sim.at(t_trip, _trip, label=label or f"IED_trip[{ied_label}]")



def trigger_ied_cb_close(sim: "Simulation",
                         ied: Union["IED", str],
                         t0: float,
                         *,
                         clear_trip: bool = True,
                         clear_mode: Literal["same_step", "prev_step"] = "prev_step",
                         label: str | None = None) -> None:
    """
    Close an IED's associated CB at (snapped) time t0.

    clear_trip:
      - If True, clears PTRC.trip (and _prev_trip if present) to avoid immediate re-open.

    clear_mode:
      - "prev_step" (default): clear at k-1, close at k
      - "same_step": clear at k, close at k (still before tick)
    """
    step0 = round(t0 / sim.dt)
    t_close = round(step0 * sim.dt, 9)
    t_clear = round(max(step0 - 1, 0) * sim.dt, 9)
    ...
    def _clear_trip(s: "Substation"):
        the_ied = _resolve_ied(s)
        if getattr(the_ied, "ptrc", None) is not None:
            the_ied.ptrc.set_trip(False)
        if hasattr(the_ied, "_prev_trip"):
            the_ied._prev_trip = False

    def _close_cb(s: "Substation"):
        the_ied = _resolve_ied(s)
        if getattr(the_ied, "xcbr", None) is None:
            raise ValueError(f"IED {the_ied.name!r} has no XCBR/CB attached; cannot close breaker.")
        the_ied.xcbr.close()

    ied_label = ied if isinstance(ied, str) else getattr(ied, "name", "IED?")
    if clear_trip:
        sim.at(t_clear if (clear_mode == "prev_step" and step0 > 0) else t_close,
               _clear_trip,
               label=f"IED_clear_trip[{ied_label}]")

    sim.at(t_close, _close_cb, label=label or f"IED_close_cb[{ied_label}]")
...
def trigger_busbar_protection(sim: "Simulation",
                              ...
                              t0: float,
                              *,
                              open_same_step: bool = True,
                              label: str | None = None) -> None:
    """
    Busbar protection event at 1 Hz resolution.

    At (snapped) time t0:
      - Assert PTRC.trip=True for ALL IEDs on the busbar.
      - Optionally open ALL associated breakers in the SAME step (default True).

    Rationale:
      - With 1 row = 1 s, busbar trip should appear simultaneous in the CSV.
      - Any 'GOOSE comm delay' should be represented in C/network behavior, not as a 1 s lag here.
    """
    step0 = round(t0 / sim.dt)
    t_evt = round(step0 * sim.dt, 9)

    def _operate(s: "Substation"):
        if ied_name not in s.ieds:
            raise KeyError(f"IED {ied_name!r} not found in substation.")
        main = s.ieds[ied_name]
        if getattr(main, "bb", None) != busbar_name:
            raise ValueError(
                f"IED {ied_name!r} is on busbar {getattr(main, 'bb', None)!r}, not {busbar_name!r}"
            )

        # Trip everyone on the busbar
        ...
        # For busbar protection, opening in the same timestep is usually the best 1 Hz abstraction.
        if open_same_step:
            for nm, ied in s.ieds.items():
                if getattr(ied, "bb", None) != busbar_name:
                    continue
                if getattr(ied, "xcbr", None) is not None:
                    ied.xcbr.open()
                # Also reset delayed-open memory to avoid weird “open again next tick”
                if hasattr(ied, "_prev_trip"):
                    ied._prev_trip = False

    sim.at(t_evt, _operate, label=label or f"BBAR_trip[{busbar_name}]")
